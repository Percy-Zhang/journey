...
export default function MonthCard(month: string, year: string, journal: Journal) {
	...
	const numEntries = useMemo(() : string => {
		let numEntries = 0
		numEntries += Object.keys(journal[year][month]).length
		return numEntries.toString()
	}, [journal[year][month]])
	...
	const hasImportant = parseInt(numImportant) > 0
	...
	return (
		<TouchableOpacity key={month} onPress={onPress} style={styles.monthContainer}>
			...
			<View style={styles.infoTextWrapper}>
				<Text>{numEntries} day{plural && 's'} recorded</Text>
				{hasImportant && <Text>{numImportant} {`\u2605`}</Text>}
			</View>
		</TouchableOpacity>
	)
}